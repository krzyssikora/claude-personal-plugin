---
name: plan-review
description: Iteratively review an implementation plan at `docs/superpowers/plans/<...>.md` via a fresh-context subagent loop. Apply catches you agree with, push back on weak ones, commit each round, stop on a clean verdict or a safety cap. Invoke after `writing-plans` writes a fresh plan, or on demand via `/plan-review [path]`.
---
...
- Right after the `writing-plans` skill writes a new plan under `docs/superpowers/plans/`.
- When the user runs `/plan-review` (with or without a path argument).
- When you've materially revised an existing plan and want an independent review pass.

Do NOT use this skill to review code, specs, or non-plan markdown. The reviewer prompt is tuned for the implementation-plan genre.

## Resolve the target plan

1. Read the `plan` argument the skill was invoked with. The slash command forwards `$ARGUMENTS` verbatim; it is a single string with no tokenization.
...
3. If the trimmed string is non-empty, treat it as a single path:
   ...
   - If the path does not exist → emit `Status: resolve-failed` with `Error: plan not found: <path>` (see the "Render the final report" section) and stop.
   - If the path is a directory → emit `Status: resolve-failed` with `Error: plan path must be a file: <path>` and stop.

4. If the trimmed string is empty (no argument):
   - Use the `Glob` tool with pattern `docs/superpowers/plans/**/*.md` from the current working directory.
   ...
   - If `Glob` returns zero results → emit `Status: resolve-failed` with `Error: no plans found under <cwd>/docs/superpowers/plans/; pass a path explicitly` and stop.
...
5. Emit the resolved path to the user (your assistant-turn output) as a markdown inline-code line so backslashes and underscores don't get mangled:
   ...
   `Resolved plan: \`<absolute-path>\``

6. Continue to the git precondition checks.

## Check git preconditions

All git operations target the **repository containing the plan**, not necessarily the parent's cwd. Compute `repo = git -C <dirname(plan_path)> rev-parse --show-toplevel` and cache it. Use `git -C "<repo>" ...` everywhere below.

Run these checks in order; on any failure, emit `Status: precondition-failed` with the corresponding `Error: ...` message and stop:

1. **In a git repo:** if `git -C <dirname(plan_path)> rev-parse --show-toplevel` fails → `Error: plan is not inside a git repository; per-round commits cannot be created`.

2. **Branch checked out:** if `git -C <repo> rev-parse --abbrev-ref HEAD` returns `HEAD` (detached state) → `Error: plan's repository is in detached-HEAD state; check out a branch before running /plan-review`.

3. **No in-progress operation:** for each of `rebase-merge`, `rebase-apply`, `MERGE_HEAD`, `CHERRY_PICK_HEAD`, check whether `git -C <repo> rev-parse --git-path <name>` resolves to an existing path. If any does → `Error: plan's repository has an in-progress rebase/merge/cherry-pick; finish or abort it first`.

4. **Plan is clean:** if `git -C <repo> diff --quiet -- <plan_path>` fails (working-tree diff) OR `git -C <repo> diff --quiet --cached -- <plan_path>` fails (index diff) → `Error: plan has uncommitted changes; commit, stash, or — if the diff is only line-endings on a fresh clone — run "git add --renormalize <plan>". The per-round commit trail needs the plan at a known baseline.`

5. **No unrelated staged changes:** if `git -C <repo> diff --cached --name-only` lists any file other than `<plan_path>` (relative to repo root) → `Error: unrelated staged changes present; commit or stash them before running /plan-review`.

Unstaged changes to files other than the plan are allowed. The parent only stages the plan file.

After preconditions pass, capture `start_sha = git -C <repo> rev-parse HEAD` (a baseline for the final report's Commits range), and compute `topic` from the plan filename per the "Topic derivation" subsection below.

## Topic derivation

`topic` is used in commit messages as `plan(<topic>): ...`.

1. Take the plan filename stem (filename minus the `.md` extension).
...
Examples:
- `2026-05-16-spec-review-skill.md` → `spec-review-skill`
...
- `notes` — free-form one-line strings (zero-match rebuttal removals, weak-rebuttal dismissals, stray-REBUTTAL prefixes, hook-rewrote-plan detection, etc.).
- `round = 1`
...
- `plan_path` (the absolute path)
- `repo` (the git toplevel containing the plan)
...
2. Substitute `<PLAN_PATH>` with the absolute `plan_path`.
...
4. **Pre-dispatch sanity assertion:** scan the rendered prompt for the literal strings `<PRIOR_ROUNDS_BLOCK>`, `</PRIOR_ROUNDS_BLOCK>`, `<PLAN_PATH>`. If any is present, this is a parent-side bug — emit `Status: template-bug` with `Error: template substitution leaked block tags or the <PLAN_PATH> placeholder into the rendered prompt` and stop.
...
2. Snapshot `S0 = read(plan_path)` (the round-start plan content as a string).
...
- Decide `parent_judges_catch_correct_or_uncertain(catch)`:
  - True if you agree the catch identifies a real gap, OR you're uncertain whether the gap is real but addressing it will not harm the plan.
  - False only when you have positive reason to believe the catch is wrong (factually, scope-wise, or a misreading).
  - **Asymmetric rule:** if your planned remediation primarily *removes* plan content rather than adds/refines, flip the uncertainty branch — apply only on positive agreement. (Does not apply to compelling rebuttals; those bypass this rubric.)
...
1. Read the current on-disk plan (this may differ from `S0` if previous catches in this round have made edits).
2. Locate the change site using `catch.where` as the primary anchor, with `catch.whats_wrong` and `catch.address` as guides. The reviewer was asked to quote in `where`, but may have paraphrased — author your own `old_string` from the current plan content.
...
`brief_description(catch)`: a one-line summary in *your* voice (not the reviewer's wording), focused on the change you applied. Aim for ~100 characters. Example: `"split phase 2 into ordered subtasks; pinned the migration step before the backfill"`.
...
```
git -C "<repo>" add "<plan_path>"
git -C "<repo>" commit -m "plan(<topic>): round <round> revisions (applied <k>, disputed <m>, stale <s>)"
```
...
After a successful commit, check whether a hook rewrote the plan without re-staging: run `git -C "<repo>" diff --quiet HEAD -- "<plan_path>"`. If it fails (i.e., there IS a diff), append a note: `round <round>: a commit hook modified the plan but did not re-stage; subsequent rounds read the modified content from disk`. (A hook that staged its rewrite produces no diff and needs no note.)
...
Render the `Commits:` section by running `git -C "<repo>" log --reverse --format='%h %s' <start_sha>..HEAD -- "<plan_path>"` and taking each output line as a bullet. If the command returns no lines, render `(none)`.
